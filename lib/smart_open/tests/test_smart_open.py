...
class SmartOpenReadTest(unittest.TestCase):
    ...
    # @responses.activate
    # def test_http_read_gz(self):
    #     """Does http read method work correctly for gzip"""
    #     CURR_DIR = os.path.abspath(os.path.dirname(__file__))
    #     zip_file = CURR_DIR.join(self.CURR_DIR, 'test_data/crlf_at_1k_boundary.warc.gz')
    #     with open(zip_file) as zip_file_handle:
    #         data = zip_file_handle.read()
    #         responses.add(responses.GET, "http://127.0.0.1/index.gz", body=data)
    #         smart_open_object = smart_open.HttpOpenRead(smart_open.ParseUri("http://127.0.0.1/index.gz"))
    #         text = smart_open_object.read().decode("utf-8")
    #         self.assertTrue(text.startsWith("WARC/1.0"))
